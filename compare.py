...
def comparePixels(pixel1,pixel2):
	red = pixel1[1][0]-pixel2[1][0]
	green = pixel1[1][1]-pixel2[1][1]
...
def findMatchingPixel(pixel1,image2Pixels):
	...
	currentClosestDist = None
	tempDist = 0
	for pixel2 in image2Pixels:
		tempDist = comparePixels(pixel1,pixel2)
		if(currentClosestDist == None or tempDist < currentClosestDist):
...
def compareImages(image1,image2):
	...
	image1Pixels = dice(image1,squareSize = 25) # using 100 right now, but have to find a better way to determine squareSize
	image2Pixels = dice(image2,squareSize = 25)
	match = -1
	dist = -1
	color = 120
